...
argsclass(Ins *i0, Ins *i1, AClass *ac, int op, AClass *aret, Ref *env)
{
	...
	if (varc && envc)
		err("win64 abi does not support variadic env calls");
...
{
	if (KBASE(ty) == 0)
		return TMP(amd64_win64_rsave[(*ni)++]);
	else
		return TMP(XMM0 + (*ns)++);
}
...
selcall(Fn *fn, Ins *i0, Ins *i1, RAlloc **rap)
{
	...
	for (stk=0, a=&ac[i1-i0]; a>ac;)
		if ((--a)->inmem) {
			if (a->align > 4)
				err("win64 abi requires alignments of 16 or less");
			stk += a->size;
			if (a->align == 4)
				stk += stk & 15;
		}
...
selpar(Fn *fn, Ins *i0, Ins *i1)
{
	...
	fn->reg = amd64_win64_argregs(CALL(fa), 0);
